...
/**
 * Cómo decora ChatGPT las líneas del entregable que pide.
 *
 * Cada campo va como ítem de lista: es el formato en el que ChatGPT devuelve
 * estructura de manera más estable, y coincide con las listas numeradas que
 * este adaptador ya usa para todo lo procedimental.
 *
 * `cita` va en prosa: ChatGPT no trata las etiquetas como referencias, así que
 * un `<convenciones_de_texto>` suelto se le aparecería como texto literal a
 * copiar en el entregable.
 */
...
function mecanicaDePlataforma(ctx: PromptContext): string {
  ...
  const confirmado =
    mecanica.confirmado.length > 0
      ? lines([
          ...
          bullets(
            mecanica.confirmado.map((d) => `**${d.etiqueta}:** ${d.valor}`)
          ),
        ])
  ...
  return lines([
    `## Cómo funciona ${ctx.plataformaPrincipalLabel} (revisado: ${mecanica.revisada})`,
    confirmado,
    discutido,
  ]);
}
...
function restriccionesDuras(ctx: PromptContext, mision: MisionDeTanda): string {
  ...
  return [
    ...
    bullets([
      ...
      `${formatoLinea} Nada que necesite más de «${ctx.tiempoPorPiezaLabel.toLowerCase()}» por pieza. Cada pieza usa el equipo que mejor le sirva, sin superar ninguno de estos niveles: ${equipoListado}.`,
      mision.reglaCTA,
...
function continuidad(semana: number): string | null {
  ...
  return [
    "## Continuidad",
    `Mira el bloque de estado con el que cerraste la Semana ${semana - 1}. No repitas ninguno de esos ganchos, ángulos ni ejemplos. Esta semana avanza sobre la anterior, no la recicla.`,
  ].join("\n");
}
...
function buildParTitulo(ctx: PromptContext, video: number): string {
  ...
  return sections([
    ...
    "**Antes que nada: todavía no escribas el guion.** Seguimos en la misma conversación, con el mismo contexto del primer mensaje. No lo repitas ni lo resumas.",
    ...
    lines([
      "## Cierre",
      "Después de los 5, marca el que escogerías y por qué, en una línea. Y para ahí: yo elijo el definitivo y te lo digo. **No escribas el guion en esta respuesta.**",
    ]),
  ]);
}
...
function buildAngulos(ctx: PromptContext, semana: number): string {
  ...
  return sections([
    ...
    "**Antes que nada: No escribas guiones todavía. En este turno solo se idea.** Seguimos en la misma conversación, con el mismo contexto del primer mensaje. No lo repitas ni lo resumas.",
    ...
    lines(["## Cierre", "Después de los 12, marca los 3 que tú escogerías y por qué, en una línea cada uno. Y para ahí: yo elijo los 3 definitivos y te los digo. **No escribas guiones en esta respuesta.**"]),
  ]);
}
