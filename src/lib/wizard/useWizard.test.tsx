...
describe("useWizard", () => {
  ...
  it("skips the oferta step for the autoridad path and reaches summary", async () => {
    ...
    act(() => result.current.goNext()); // gancho -> inventario
    act(() => result.current.goNext()); // inventario -> summary
  ...
  it("goBack from summary returns to the last step", async () => {
    ...
    expect(result.current.currentStep?.id).toBe("inventario");
  });
  ...
  it("resumes on the result screen after a reload", async () => {
    ...
    for (let i = 0; i < 9; i++) act(() => first.result.current.goNext());
